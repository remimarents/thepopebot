...
async function runInteractiveContainer({ containerName, repo, branch, codingAgent, featureBranch, workspaceId, injectSecrets, continueSession = true }) {
  ...
  const image = process.env.AGENT_JOB_IMAGE_URL || `stephengpope/thepopebot:coding-agent-${agent}-${version}`;
...
function buildAgentAuthEnv(agent) {
  ...
  if (agent === 'claude-code') {
    ...
    if (builtinKeyMap[provider]) {
      ...
      if (custom) {
        const apiKey = custom.apiKey || 'ollama';
        // OpenCode/Pi expect OpenAI-style env vars for custom providers
        env.push(`OPENAI_BASE_URL=${custom.baseUrl}`);
        env.push(`OPENAI_API_KEY=${apiKey}`);
        // Back-compat for any tooling that reads custom env names
        env.push(`CUSTOM_OPENAI_BASE_URL=${custom.baseUrl}`);
        env.push(`CUSTOM_API_KEY=${apiKey}`);
...
async function runClusterWorkerContainer({ containerName, image, codingAgent, env = [], binds = [], workingDir }) {
  const version = process.env.THEPOPEBOT_VERSION;
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const resolvedImage = image || process.env.AGENT_JOB_IMAGE_URL || `stephengpope/thepopebot:coding-agent-${agent}-${version}`;
