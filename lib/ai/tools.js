...
const agentJobTool = tool(
  async ({ prompt }, runtime) => {
    const resolvedPrompt = prompt || runtime?.configurable?.lastUserMessage || '';
    if (!resolvedPrompt.trim()) {
      return JSON.stringify({ success: false, error: 'Missing prompt for agent job.' });
    }
    const result = await createAgentJob(resolvedPrompt);
...
const agentChatCodingTool = tool(
  async ({ prompt }, runtime) => {
    try {
      const resolvedPrompt = prompt || runtime?.configurable?.lastUserMessage || '';
      if (!resolvedPrompt.trim()) {
        return JSON.stringify([{ type: 'error', message: 'Missing prompt for coding agent.' }]);
      }
